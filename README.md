...
Omit `original_symbol` to resolve the exchange/quote instrument from CoinGlass's public ticker metadata. When several instruments share the same exchange/base/quote (perpetual plus dated deliveries), the client prefers the conventional `{symbol}{quote}` ticker (e.g. `BTCUSDT`, matching live pair smoke) **only when it is itself a perpetual** or no perpetual conflicts; otherwise a unique `type==1` perpetual wins. A conventional name that is actually a dated delivery never silently binds the heatmap while a real perpetual exists. Truly unknown or still-ambiguous matches fail closed; no fallback manufactures a fake `COINUSDT`. Models 1/2/3 and pair/aggregate scopes use separate endpoints. Legacy v3 is an explicit compatibility mode, not a numbered current model.
...
- **Long/short:** taker volume, account shares and position ratios are distinct, not counts of people. Coinalyze `ls` is the venue-reported long/short **account percentages** (`ls_unit=percent_of_accounts`), `lsr` the upstream ratio field; neither is a position-notional split.
- **Options:** Deribit output is OI distribution, not gamma exposure/dealer positioning. `deribit_walls(expiry_policy="max_oi")` honestly selects maximum OI; `nearest` and an explicit `expiry` are alternatives. `reference_price`/`spot` is Deribit's `estimated_delivery_price` (index estimate), not a separately fetched live spot quote (`reference_price_kind`).
...
The liquidation collector also writes operational health to `LIQ_STATE_DIR/_health/status.json` using atomic replace. This file is intentionally outside the root symbol-file glob. It reports each source's connected/ACK/error state, last accepted event time when one has actually arrived, pending buffer count, accepted record count, flush errors and last flush/persist timestamps. A quiet but connected feed is different from a failed ACK or dead source; no synthetic liquidation events are created to prove liveness. Source `errors` are cumulative for the process lifetime — reconnects reset `connected`/`acknowledged` but never zero the error counter, so a flapping subscription stays visible. Bybit V5 and OKX heartbeats use their required app-level ping messages; a protocol ping alone is not honoured by either venue.
